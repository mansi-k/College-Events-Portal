...
if(isset($_SESSION['user']))
{
	if(isset($_POST['changepass']))
	{
		...
		if($newpass==$cfnewpass)
		{
			if(strlen($newpass)>=6)
			{
				$sql_query_old = mysqli_query($bd,"SELECT password FROM student WHERE id=".$_SESSION['user']);
				$old=mysqli_fetch_array($sql_query_old);
				if($old['password']== $oldpass)
				{
					$sql_query_up = "UPDATE student SET password='$newpass' WHERE id=".$_SESSION['user']." and password='$oldpass'";
					if(mysqli_query($bd,$sql_query_up))
					{
						?>
