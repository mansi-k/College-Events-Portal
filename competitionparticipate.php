...
{
	$wname=$_POST['w_part'];
	$sql_query=mysqli_query($bd,"INSERT INTO `competition_part` (`id`, `act_name`, `stud_id`, `stud_name`, `branch`, `year`, mobile_no, `paid`, part_date) VALUES (NULL, '$wname', '$login_id', '$full_name', '$branch', '$year', '$mobile_no', 'no', CURDATE());");
}

if((isset($_POST['w_nopart'])))
{
	$wname=$_POST['w_nopart'];
	$sql_query=mysqli_query($bd,"DELETE FROM competition_part WHERE stud_id='$login_id' AND act_name='$wname'");
}
?>
...
<center>
        <form method="post">
            ...
            <?php
			$sql_query=mysqli_query($bd,"SELECT * FROM competition WHERE (branch='$branch' OR branch='all' OR branch='$actbranch') AND (year='$year' OR year='all')");
			if(mysqli_num_rows($sql_query)>0)
			{
				while($row=mysqli_fetch_row($sql_query))
				{
                    ...
                    <?php 
                    $res=mysqli_query($bd,"SELECT * FROM competition_part WHERE act_name='$workn' AND stud_id='$login_id'");
